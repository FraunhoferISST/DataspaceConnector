...
CREATE TABLE public.resource (
    ...
    language character varying(255),
    licence character varying(2048),
...
COPY public.artifact (dtype, id, bootstrap_id, created_date, deleted, modified_date, automated_download, byte_size, check_sum, num_accessed, remote_address, remote_id, title, data_id) FROM stdin;
\.
...
COPY public.contract (id, bootstrap_id, created_date, deleted, modified_date, consumer, contract_end, provider, remote_id, contract_start, title) FROM stdin;
\.
...
COPY public.contractrule (id, bootstrap_id, created_date, deleted, modified_date, remote_id, title, value) FROM stdin;
\.
...
COPY public.data (dtype, id, deleted, value, access_url, password, username) FROM stdin;
\.
...
COPY public.representation (id, bootstrap_id, created_date, deleted, modified_date, language, media_type, remote_id, standard, title) FROM stdin;
\.
...
COPY public.resource (dtype, id, bootstrap_id, created_date, deleted, modified_date, description, endpoint_documentation, language, licence, publisher, sovereign, title, version, remote_id) FROM stdin;
\.
