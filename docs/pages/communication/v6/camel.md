...
**Note**: The full name of the driver class is required, so e.g. *org.h2.Driver* or *org.postgresql.Driver*.
...
The above examples show simple routes which only call one endpoint. But using Camel routes to
...
downloaded and deployed an app, which has one input and one output endpoint (meaning you
...
Now you can link the route to an artifact as described in the previous chapters. When you request
