...
class DescriptionRequestHandlerTest {
    ...
    public void handleMessage_validSelfDescriptionMsg_returnSelfDescription() throws DatatypeConfigurationException {
        ...
        /* ASSERT */
        final var expected = (BodyResponse) handler.constructConnectorSelfDescription(message.getIssuerConnector(), message.getId());

        // Compare payload
        assertEquals(expected.getPayload(), result.getPayload());

        // Compare header
        assertEquals(expected.getHeader().getIssuerConnector(), result.getHeader().getIssuerConnector());
        assertEquals(expected.getHeader().getAuthorizationToken(), result.getHeader().getAuthorizationToken());
        assertEquals(expected.getHeader().getComment().toString(), result.getHeader().getComment().toString());
        assertEquals(expected.getHeader().getCorrelationMessage(), result.getHeader().getCorrelationMessage());
        assertEquals(expected.getHeader().getContentVersion(), result.getHeader().getContentVersion());
        assertEquals(expected.getHeader().getLabel().toString(), result.getHeader().getLabel().toString());
        ...
        assertEquals(expected.getHeader().getRecipientAgent(), result.getHeader().getRecipientAgent());
        assertEquals(expected.getHeader().getRecipientConnector(), result.getHeader().getRecipientConnector());
        assertEquals(expected.getHeader().getSenderAgent(), result.getHeader().getSenderAgent());
        assertEquals(expected.getHeader().getTransferContract(), result.getHeader().getTransferContract());
    }

    @Test
    public void handleMessage_validResourceDescriptionMsgKnownId_returnResourceDescription() throws DatatypeConfigurationException {
        /* ARRANGE */
        ...
        Mockito.when(resolver.getEntityById(Mockito.eq(message.getRequestedElement()))).thenReturn(artifact);

        /* ACT */
        final var result = (BodyResponse)handler.handleMessage((DescriptionRequestMessageImpl) message, null);

        /* ASSERT */
        final var expected = (BodyResponse) handler.constructResourceDescription(message.getRequestedElement(), message.getIssuerConnector(), message.getId());
    ...
    @Test
    public void handleMessage_unsupportedMessage_returnUnsupportedVersionRejectionMessage() throws DatatypeConfigurationException {
        /* ARRANGE */
        final var calendar = new GregorianCalendar();
        calendar.setTime(new Date());
        final var xmlCalendar = DatatypeFactory.newInstance().newXMLGregorianCalendar(calendar);
    ...
    @Test
    public void handleMessage_validResourceDescriptionMsgUnknownId_returnNotFoundRejectionReason() throws DatatypeConfigurationException {
        /* ARRANGE */
        final var calendar = new GregorianCalendar();
        calendar.setTime(new Date());
        final var xmlCalendar = DatatypeFactory.newInstance().newXMLGregorianCalendar(calendar);
        ...
        Mockito.when(resolver.getEntityById(Mockito.eq(message.getRequestedElement()))).thenThrow(ResourceNotFoundException.class);
