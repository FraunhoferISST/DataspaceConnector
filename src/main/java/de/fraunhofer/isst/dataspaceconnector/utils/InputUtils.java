...
public final class InputUtils {
     ...
     */
    private static void validateQueryInput(final QueryInput queryInput) throws IllegalArgumentException {
        if (queryInput != null && queryInput.getHeaders() != null) {
            for (Map.Entry<String, String> entry: queryInput.getHeaders().entrySet()) {
                if (entry.getKey() == null || entry.getKey().trim().isEmpty()
                    ...
                    throw new IllegalArgumentException("Header key or value should not be null, "
                            + "blank or empty (key:" + entry.getKey() + ", value: " + entry.getValue() + ").");
                }
            }
        }
